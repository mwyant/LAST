...
from app import transcribe_functions as tf
...
CHUNK_SECONDS_DEFAULT = 300  # 5 minutes per chunk
...
# ---- Helpers for chunk timestamp shifting ----
...
def transcription_worker(job_id: str, remove_input_after: bool = True) -> None:
    ...
    try:
        ...
        # Denoise/enhance (blocking; we're in a worker thread)
        ...
        # split denoised master into chunks (48k WAVs)
        workdir = Path(tempfile.mkdtemp(prefix=f"proc_{job.id}_"))
        try:
            segs = denoise_utils.split_into_chunks(str(denoised_out), str(workdir), seg_secs=CHUNK_SECONDS_DEFAULT)
            if not segs:
                # If split failed or produced nothing, fallback to single segment (the denoised_out itself)
                segs = [str(denoised_out)]

            job.progress = 50.0
            job.message = f"split into {len(segs)} chunk(s); transcribing..."
            push_job_event(job, "progress", {"progress": job.progress, "message": job.message})

            combined_plain_parts: List[str] = []
            combined_lines: List[str] = []

            total_chunks = max(1, len(segs))
            for i, seg in enumerate(segs):
                offset = i * CHUNK_SECONDS_DEFAULT
                # Transcode this chunk to 16k mono for ASR
                chunk16 = workdir / f"{job.id}_chunk_{i}_16k.wav"
                job.transcode_path = str(chunk16)

                def _chunk_progress(pct: float, msg: str, idx=i, total=total_chunks):
                    # map chunk progress into overall job progress range [55..90]
                    overall = 55.0 + ((idx + (pct / 100.0)) / float(total)) * 35.0
                    job.progress = overall
                    job.message = f"chunk {idx+1}/{total} - {msg}"
                    push_job_event(job, "progress", {"progress": job.progress, "message": job.message})

                transcode_to_wav(Path(seg), chunk16, progress_cb=_chunk_progress)
                ...
                push_job_event(job, "progress", {"progress": job.progress, "message": f"transcribing chunk {i+1}/{total_chunks}"})
                start_t = time.time()
                out = run_transcribe_model(MODEL["instance"], str(chunk16))
                elapsed = time.time() - start_t

                # extract object/text/lines
                transcript_obj = out[0] if isinstance(out, (list, tuple)) and len(out) > 0 else out
                plain_chunk, lines_chunk = transcript_to_human_readable(transcript_obj)
                # shift line timestamps by offset seconds
                shifted_lines = [_shift_line_time_prefix(ln, offset) for ln in lines_chunk]
                combined_plain_parts.append(plain_chunk)
                combined_lines.extend(shifted_lines)

                # update progress after chunk
                job.progress = 55.0 + ((i + 1) / float(total_chunks)) * 35.0
                job.message = f"completed chunk {i+1}/{total_chunks} (took {elapsed:.1f}s)"
                push_job_event(job, "progress", {"progress": job.progress, "message": job.message})

            # combine plain text
            full_plain = " ".join([p for p in combined_plain_parts if p])

            # diarize using denoised master (preferred), else fallback
            labeled_lines: List[str]
            try:
                diary_mod = importlib.import_module("app.diary")
                segments = diary_mod.diarize(str(denoised_out))
                labeled_lines = diary_mod.apply_diarization_to_lines(combined_lines, segments)
            except Exception:
                try:
                    segments = fallback_diarize(str(denoised_out))
                    labeled_lines = apply_segments_to_lines(combined_lines, segments)
                except Exception:
                    logger.exception("Diarization failed; falling back to plain lines")
                    labeled_lines = combined_lines
            ...
            if remove_input_after:
                try:
                    ...
                    pass
                try:
                    transcode_path = UPLOAD_DIR / f"{job.id}-16k.wav"
...
@app.get("/denoise/job/{job_id}")
def denoise_by_job(job_id: str):
    """
    Return the denoised master file for a given job if it exists.
    Path: OUT_DIR / f"{job_id}_denoised.wav"
    """
    path = OUT_DIR / f"{job_id}_denoised.wav"
    if not path.exists():
...
# New transcription with chunking to handle large files
@app.post("/transcribe")
async def transcribe_start(payload: Dict[str, Any]):
    job_id = payload.get("job_id")
    if not job_id:
        raise HTTPException(status_code=400, detail="job_id required")
    use_denoise = bool(payload.get("use_denoise", False))
    with JOBS_LOCK:
        job = JOBS.get(job_id)
    if not job:
        raise HTTPException(status_code=404, detail="job not found")
    if job.status in ("running", "done"):
        return JSONResponse({"job_id": job.id, "status": job.status})
    loop = asyncio.get_event_loop()
    # pass use_denoise into the worker (defaults to False)
    loop.run_in_executor(EXECUTOR, transcription_worker, job.id, use_denoise)
    job.status = "queued"
    push_job_event(job, "status", job.to_dict())
    return JSONResponse({"job_id": job.id, "status": "queued"})
